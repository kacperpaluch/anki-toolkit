"""CometAPI provider (OpenAI-compatible endpoint)."""
...
class CometAPIProvider(BaseProvider):
    def call_api(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            if not choices or not isinstance(choices, list):
                self.last_error = f"CometAPI: unexpected response structure: {list(res_data.keys())}"
                self.logger.error(self.last_error)
                return None
            message = choices[0].get("message")
            if not message:
                self.last_error = "CometAPI: missing 'message' in first choice"
                self.logger.error(self.last_error)
                return None
            content = message.get("content")
            if content is None:
                self.last_error = (
                    "CometAPI: missing 'content' in message "
            ...
            if text is None:
                self.last_error = f"CometAPI: unsupported message content type: {type(content).__name__}"
                self.logger.error(self.last_error)
                return None
            return text
        except (KeyError, IndexError, TypeError) as e:
            self.last_error = f"CometAPI response parse error: {e}"
            self.logger.error(self.last_error)
            return None
        except Exception as e:
            self.last_error = f"CometAPI Error: {e}"
            self.logger.error(self.last_error)
            return None
