...
class OpenAIProvider(BaseProvider):
    def call_api(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            if not message:
                self.last_error = "OpenAI: missing 'message' in first choice"
                self.logger.error(self.last_error)
                return None
            content = message.get("content")
            if content is None:
                self.last_error = (
                    "OpenAI: missing 'content' in message "
            ...
            if text is None:
                self.last_error = f"OpenAI: unsupported message content type: {type(content).__name__}"
                self.logger.error(self.last_error)
                return None
            return text
        except (KeyError, IndexError, TypeError) as e:
            self.last_error = f"OpenAI response parse error: {e}"
            self.logger.error(self.last_error)
            return None
