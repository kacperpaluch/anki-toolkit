...
| `batch_backfill.py` | Backfill przez **Batch API** (async, ~50% taniej) dla `anthropic` i `openai` — osobny od ścieżki sync, bez fallbacku. Rdzeń niezależny od dostawcy (`build_items`/`summarize`/`submit`/`poll_results`/`apply_results`, wspólny `iter_note_fields`, `custom_id="i{n}"`, prompt ze stanu z chwili wysyłki) + cienkie backendy: **Anthropic** (inline requests, `results_url`, temperature pominięte) i **OpenAI** (upload JSONL przez Files API + `create batch(input_file_id)` + download `output_file_id`; batch per model, bo API wymaga jednego modelu). `submit` tworzy osobny rekord na backend/model; `poll_results` dispatch po `record["provider"]` zwraca **znormalizowany** wynik; `apply_results` (main thread) pisze **tylko w puste pola**, dedup per nid, brakujące wyniki jako `failed`. Persystencja `user_files/ai_batches.json` (`in_progress`/`applied`, rekord z polem `provider`) |
