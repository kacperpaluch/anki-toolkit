...
| `browser_ui.py` | UI przeglądarki — najpierw pozycje workflowów (po jednej na workflow z `config["workflows"]` → `_on_workflow_browser`), potem submenu `Generuj pola ▸` („Wszystkie puste" + per-pole „AI: def" spłaszczone po nazwie pola docelowego) i `Generuj zablokowane ▸` (pola `manual_only`); batch z natywnym paskiem `mw.progress` przez wspólny helper `common.progress` (`start_progress`/`update_progress`/`finish_progress`, `want_cancel()`) zamiast własnego QProgressDialog — trzyma Anki „busy", więc automatyczny backup/sync się odkłada i nie zasłania przycisku Anuluj; po zapisie `_save_changed_notes(browser, changed_notes, summary)` woła `deck_router.route_after_edit` (soft-import); cancel_flag; `_run_batch(only_fields=...)` i `_on_workflow_browser` — notatki wczytywane na głównym wątku (kolekcja Anki jest jednowątkowa), workery (`ThreadPoolExecutor(parallel_requests)`, chunki po `batch_limit`) tylko mutują je w pamięci, per-notatka własny `FieldGenerator`; batch pól zawsze `overwrite=False` (pomija wypełnione); dawne hardcodowane pipeline'y („Generuj wszystko…", „Rozdziel + generuj naukę…") są teraz zwykłymi workflowami seedowanymi przez `migrate_workflows()`. Submenu `Batch API ▸` — `_on_batch_submit(only_fields=...)` (pola auto + `manual_only`) tworzy zadanie (`add_job`); `check_pending_batches(silent)` (poll → `apply_results` → `mark_applied` **dopiero po commicie** `CollectionOp` → `_advance_jobs` dosyła kolejny plaster) wołane z `profile_did_open` i timera co 5 min; `_advance_jobs` buduje itemy notatka-po-notatce i **przerywa na `openai_budget_left()`** (nie renderuje promptów całej reszty joba co tick), wychodzi od razu gdy budżet = 0 (kolejka pełna / backoff), guard `_advance_running` chroni przed równoległymi przebiegami (timer + ręczny klik), błędy z `submit` logowane i pokazywane w tooltipie, tooltip sukcesu z postępem zadania (`record_job_progress`), po `mark_applied` w tle `cleanup_openai_files` |
| `batch_backfill.py` | Backfill przez **Batch API** (async, ~50% taniej) dla `anthropic` i `openai` — osobny od ścieżki sync, bez fallbacku. Rdzeń niezależny od dostawcy (`build_items`/`summarize`/`submit`/`poll_results`/`apply_results`, wspólny `iter_note_fields` → **do batcha idą tylko puste pola** (`overwrite=False`), `custom_id="i{n}"`, prompt ze stanu z chwili wysyłki) + cienkie backendy: **Anthropic** (inline requests, `results_url`, temperature pominięte) i **OpenAI** (upload JSONL przez Files API + `create batch(input_file_id)` + download `output_file_id`; batch per model, bo API wymaga jednego modelu). `submit` tworzy osobny rekord na backend/model; OpenAI akceptuje `create` i _asynchronicznie_ failuje batche ponad limit kolejki org (~2M), więc `_chunk_by_tokens` (poniżej `openai_batch_token_budget`, dom. 1,5M, est. `len//4 + openai_batch_output_reserve`) + wysyłka do wyczerpania budżetu **minus `_pending_openai_tokens()`** (batche w locie, pole `enq_tokens`), reszta odłożona. Create bywa też odrzucany **synchronicznie** 400-tką „Enqueued token limit" (licznik OpenAI potrafi wisieć godzinami po fali failów) — pierwszy błąd create **kończy przebieg** (każda próba marnuje upload pliku wejściowego), błąd enqueued dodatkowo ustawia **backoff 30 min** (`_openai_blocked_until`, `openai_budget_left()` zwraca wtedy 0), a plik wgrany pod nieudany create jest kasowany (`_delete_openai_file`). **Auto-dosyłanie (hands-off):** `add_job(nids, only_fields, total, sent)`/`active_jobs`/`finish_job`/`job_expired` (24h) zapisują zaznaczenie w `ai_batches.json` (`jobs`; `total`/`sent` = liczniki zapytań dla `record_job_progress(records)` → string „S/T zapytań (p%)" do tooltipa), a `inflight_fields()` ((nid,field) w oczekujących batchach) chroni przed duplikatami — cykl `browser_ui._advance_jobs` (timer co 5 min) dosyła kolejne plastry pustych pól. `poll_results` dispatch po `record["provider"]` zwraca **znormalizowany** wynik; `apply_results` (main thread) pisze **tylko w puste pola**, dedup per nid, brakujące wyniki jako `failed`, **nie** oznacza batchy — status `applied` zapisuje `mark_applied(bids)` wołany przez caller **po commicie notatek** (pad Anki przed commitem → następny poll pobierze i dopisze ponownie, idempotentnie); potem `cleanup_openai_files(records, config)` (tło) kasuje ze storage OpenAI plik wejściowy (`input_file_id` zapisany w rekordzie przy submit) i wynikowy (`output_file_id` dopisany do rekordu przy pollu). Persystencja `user_files/ai_batches.json` (`in_progress`/`applied`, rekord z polami `provider`/`enq_tokens`; lista `jobs`) |
