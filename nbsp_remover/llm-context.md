...
2. **Ręcznie** — jednorazowe czyszczenie całej kolekcji przez **Narzędzia → Anki Toolkit → Purge collection from &nbsp;**
...
| `addcards.py` | Hook na dodawanie kart — czyści pola w locie przy każdym `Add`, wywołuje `mw.col.update_note(note)` |
...
| `utils.py` | Stałe regex (`NBSP`, `DIV_TAG_RE`, `DIV_WRAP_RE`, `TRAILING_BR_RE`), `_get_config()`, `_get_skip_field()`, `_show_tooltip()`, `purge_tooltip`, `editing_tooltip` |
...
Ustawienia edytowane przez **Narzędzia → Anki Toolkit → Ustawienia... → zakładka Narzędzia** (sekcja "nbsp Remover").
...
Użytkownik klika Add w oknie dodawania kart
  → on_add(note)
      ...
      → dla każdego pola:
          → zamień &nbsp; → " " (przez str.replace, nie regex)
          → jeśli pole == skip_field:
              usuń <div[^>]*> i </div> całkowicie (regex DIV_TAG_RE)
          → pozostałe pola:
              <div[^>]*>tekst</div> → tekst<br> (regex DIV_WRAP_RE)
              usuń trailing <br> (regex TRAILING_BR_RE)
...
Rejestry regex współdzielone przez `addcards.py` i `collection.py` (zdefiniowane w `utils.py`):
- `NBSP` = `"&nbsp;"` (literal)
