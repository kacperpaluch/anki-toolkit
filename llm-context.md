...
│   ├── _generator.py
│   ├── editor_ui.py
│   ├── browser_ui.py                # batch AI (równoległy) + batch workflow (per-notatka) + Batch API Anthropic + CollectionOp
│   ├── field_generator.py
...
| `ai_generator/batch_backfill.py` | Backfill przez **Batch API** (async, ~50% taniej) dla `anthropic` i `openai` — osobny od ścieżki sync, bez fallbacku. Rdzeń niezależny od dostawcy: `build_items(notes, config, only_fields)` → `(items, skipped)` (wspólny `iter_note_fields`, `custom_id="i{n}"`, prompt ze stanu notatki z chwili wysyłki, tylko dostawcy z `_ELIGIBLE_PROVIDERS`), `summarize(items)` (podsumowanie per dostawca/model do dialogu), `submit(items, config)` grupuje i tworzy **osobny rekord na backend/model** (Anthropic: jeden batch; OpenAI: batch per model — API wymaga jednego modelu; OpenAI akceptuje `create` i _asynchronicznie_ failuje batche ponad limit kolejki org (~2M), więc dzielimy na paczki `_chunk_by_tokens` poniżej `openai_batch_token_budget` (dom. 1,5M, est. `len//4 + openai_batch_output_reserve`) i wysyłamy do wyczerpania budżetu **z odjęciem `_pending_openai_tokens()`** (naszych batchy w locie, pole `enq_tokens` w rekordzie); reszta odłożona), **auto-dosyłanie**: `add_job(nids, only_fields)` zapisuje zaznaczenie jako zadanie w `ai_batches.json`; `active_jobs`/`finish_job`/`job_expired` (24h) + `inflight_fields()` ((nid,field) w oczekujących batchach) obsługują cykl `browser_ui._advance_jobs` (timer co 5 min → `build_items` na pustych polach minus inflight → `submit` kolejnego plastra), `poll_results(config)` dispatch po `record["provider"]` → `_POLLERS` → **znormalizowany** wynik `{custom_id, ok, text, in_tok, out_tok}`, `apply_results(col, ended)` (MAIN THREAD) pisze **tylko w puste pola**, dedup per nid, brakujące wyniki (OpenAI expired) liczone jako `failed`, `stats.record_request(provider, ...)`. Backendy: Anthropic (`_submit_anthropic`/`_poll_anthropic` — inline requests, `results_url`, temperature pominięte), OpenAI (`_submit_openai`/`_poll_openai` — upload JSONL przez Files API multipart `_multipart`, `create batch(input_file_id)`, download `output_file_id`; body przez `add_temperature_if_supported`/`add_reasoning_effort_if_supported`). Rekord ma pole `provider` (stare bez niego = `anthropic`). Persystencja `user_files/ai_batches.json` (`in_progress`/`applied`). UI + auto-poll przy `profile_did_open` w `browser_ui.check_pending_batches` |
...
| `ai_generator/browser_ui.py` | UI przeglądarki — `add_to_context_menu` iteruje `get_workflows()` (każdy = jedna pozycja), potem submenu `Generuj pola ▸`/`Generuj zablokowane ▸` gated przez `context_menu` (`ai_fields`/`ai_blocked`); `_all_configured_target_fields(config, manual_only=False)` rozdziela listy pól; batch AI równoległy (per-job `FieldGenerator`, paczki batch_limit+sleep, `overwrite=False`) przez `_run_batch`; batch workflow `_on_workflow_browser(browser, workflow)` **równoległy** (`ThreadPoolExecutor(parallel_requests)`, paczki batch_limit+sleep, per-notatka `FieldGenerator` podawany do `execute_step`; notatki preloadowane na main thread, kroki per-notatka sekwencyjne); zapis zmienionych notatek jednym `CollectionOp` (`update_notes`); submenu **Batch API (Anthropic, tańszy)** — `_on_batch_submit(only_fields=...)` (build_requests + dialog `askUser` + submit w tle) dla pól auto (`target_fields`) oraz — gdy istnieją — pól `manual_only` (`manual_fields`: „…wszystkie zablokowane" + per-pole „Batch (zablokowane): `<pole>`"; jawny `only_fields` omija guard `manual_only` w `iter_note_fields`) i `check_pending_batches(silent)` (poll w tle → `apply_results` na main thread → `CollectionOp` → `_advance_jobs`); `_advance_jobs(config)` dla każdego aktywnego zadania buduje `build_items` na pustych polach, odfiltrowuje `inflight_fields()` (dedup też między zadaniami), nadaje unikalne `custom_id` i dosyła następny plaster; zadanie kończone gdy `build_items` puste albo `job_expired`. `check_pending_batches(silent=True)` wołane z `profile_did_open` **i cyklicznie co 5 min** (QTimer w `__init__.py`, referencja na `mw`); `_on_batch_submit` po udanej wysyłce woła `add_job(nids, only_fields)` |
