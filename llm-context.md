...
| `ai_generator/` | Generuje treść pól kart przez AI (7 providerów); batch w tle z grupowaniem i anulowaniem; workflow "Generuj wszystko" (AI → Słownik → TTS); `reasoning_effort` z fallbackiem | `on_editor_buttons_init`, `add_to_context_menu` |
| `tts/` | Generuje audio MP3 przez Kokoro (lokalny) lub OpenRouter TTS API; przycisk w edytorze + submenu w menu kontekstowym przeglądarki; podzielony na config/api/processor | `on_editor_buttons_init`, `add_to_context_menu` |
| `filtered_deck/` | Tworzy talię filtrowaną z dialogiem ustawień; nazwa talii i deck docelowy konfigurowalne | `setup_menu(parent_menu)` |
...
| `nbsp_remover/` | Usuwa `&nbsp;` i czyści `<div>` z pól kart; pole "skip" konfigurowalne | `setup_menu(parent_menu)` + auto-hook |
...
3. Rejestruje hooki edytora (natychmiast, bez `main_window_did_init`)
...
- **AI Generator** — klucze API (z przyciskiem Pokaż/Ukryj), modele (z przyciskiem Pobierz), temperatura, OpenAI reasoning level, limity sieci (max_retries, request_timeout), tagi wykluczające (skip_tags); podzakładka **Prompty** z dwupanelowym edytorem promptów; podzakładka **Workflow** z konfiguracją "Generuj wszystko"
- **TTS** — provider (Kokoro / OpenRouter), klucz API OpenRouter (lub współdzielony z AI), model (z przyciskiem Pobierz), głosy (checklista), zadania TTS (Dodaj/Edytuj/Usuń), speed, wydajność (max_workers, max_retries, timeout)
- **Audio Normalizer** — ścieżka ffmpeg (z przyciskiem "Sprawdź"), opcje loudnorm, liczba wątków
- **Narzędzia** — dwie sekcje jako QGroupBox: Talia filtrowana (deck_name, search_deck), nbsp Remover (show_tooltip, auto_run_startup, skip_field)
...
| `audio_normalizer` | `audio_normalizer/` | Zakładka Audio Normalizer |
| `filtered_deck` | `filtered_deck/` | Zakładka Narzędzia (sekcja Talia filtrowana) |
| `nbsp_remover` | `nbsp_remover/` | Zakładka Narzędzia (sekcja nbsp Remover) |
...
| `settings/audio_normalizer_tab.py` | Zakładka Audio Normalizer — `AudioNormalizerTab` |
...
| `ai_generator/editor_ui.py` | UI edytora — przycisk AI, workflow, async (`run_in_background` + `saveNow`); ochrona przed wyścigiem podwójnego kliknięcia (`_GENERATING`) |
| `ai_generator/workflow.py` | Workflow "Generuj wszystko" — sekwencyjne AI → Dict → TTS na pojedynczej notatce |
...
| `tts/processor.py` | Przetwarzanie notatek — `process_task_async()`, `process_single_note()`, `_collect_work_items()` |
| `tts/editor_ui.py` | Przycisk TTS w edytorze — `saveNow` + `run_in_background` + `process_single_note()` |
...
ai_generator/__init__.py        ← re-eksport: on_editor_buttons_init, add_to_context_menu
    ...
    └── editor_ui.py            ← przycisk AI + workflow button; saveNow(start) przed zadaniem, _GENERATING guard
    ...
    └── workflow.py             (AI → Dict → TTS sekwencyjnie); używa common/ADDON_NAME
...
tts/__init__.py                 ← add_to_context_menu + exports on_editor_buttons_init
    ...
    └── processor.py            (process_task_async, process_single_note, _collect_work_items, _save_batch_results); używa common/unique_filename, clean_html
    └── editor_ui.py            (przycisk TTS w edytorze); używa common/unique_filename, clean_html, unique
