...
| `filtered_deck/` | Tworzy talię filtrowaną z predefiniowanego presetu (stała `PRESETS`, QComboBox); cała kolekcja, `resched=False` | `setup_menu(parent_menu)` |
...
- **Narzędzia** — trzy sekcje jako QGroupBox: Talia filtrowana (deck_name), Czyszczenie HTML (show_tooltip, auto_run_startup, skip_field), Rozdzielanie pól (source_field, separator, target_fields jako tekst przecinkami, overwrite checkbox)
