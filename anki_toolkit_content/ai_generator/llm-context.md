...
| `workflow.py` | Nazwane workflowy (`config["workflows"]`, lista `{name, editor_button, steps}`) — `disabled_step_modules()` zatrzymuje cały przebieg przed startem, gdy wymagany moduł jest wyłączony; `execute_step()` mutuje notatkę w pamięci; `run_workflow_editor()` wykonuje sekwencyjne kroki z jednym `FieldGenerator` i wspólnym guardem edytora. |
...
Przyciski workflowów w edytorze:
  ...
  → workflow.run_workflow_editor(editor, workflow)
      → `disabled_step_modules()` zatrzymuje przebieg, jeśli dowolny wymagany moduł jest wyłączony
      ...
      → po ostatnim kroku wywołuje `editor.loadNote()` (tylko gdy edytor nadal pokazuje tę notatkę), zwalnia guard i pokazuje jedno podsumowanie
```
