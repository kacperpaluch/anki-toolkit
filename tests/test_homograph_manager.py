...
class FakeNote:
    def __init__(self, nid, word, tags=None, model="angielski"):
        self.id = nid
...
class FakeCol:
    ...
    def add_note(self, nid, word, cards, tags=None, model="angielski"):
        note = FakeNote(nid, word, tags, model)
        self.notes[nid] = note
        cids = []
        for c in cards:
            self.cards[c.id] = c
...
class HomographReactiveTests(unittest.TestCase):
    ...
    def test_reactive_does_not_gate_a_learning_meaning(self):
        # Real-world case: another meaning is already in review. Answering a
        # brand-new meaning must not suspend the one you're already studying.
        col = FakeCol()
        a = new()
        col.add_note(1, "beam", [a, new()])            # answered, brand-new
        col.add_note(2, "beam", [review(5), new()])    # already learning
        col.add_note(3, "beam", [new(), new()])        # brand-new
        s, u = logic.process_reactive(col, 1, a.id, THRESHOLD, TAG)
        self.assertEqual(s, 2)                          # only the brand-new meaning (3)
        self.assertFalse(col.get_note(2).has_tag(TAG))
        self.assertTrue(col.get_note(3).has_tag(TAG))
        n2 = [col.cards[c] for c in col.note_cards[2]]
        self.assertFalse(any(c.queue == QUEUE_TYPE_SUSPENDED for c in n2))

    def test_note_types_excludes_other_models(self):
        # Same front across two note types. Scoped to "angielski", the Prestonly
        # note must not be grouped/suspended; unscoped, it is.
        col = FakeCol()
        a = new()
        col.add_note(1, "here you are.", [a, new()], model="angielski")
        col.add_note(2, "here you are.", [new(), new()], model="Prestonly")
        s, u = logic.process_reactive(
            col, 1, a.id, THRESHOLD, TAG, note_types=["angielski"])
        self.assertEqual((s, u), (0, 0))
        self.assertFalse(col.get_note(2).has_tag(TAG))
        # No scope → both models group together, note 2's two NEW suspended.
        s2, _ = logic.process_reactive(col, 1, a.id, THRESHOLD, TAG)
...
class HomographSyncTests(unittest.TestCase):
    ...
    def test_sync_leaves_all_already_learning_meanings_alone(self):
        # Several meanings already in review before the module existed. None of
        # them may be suspended/tagged; only the brand-new meaning is gated.
        col = FakeCol()
        col.add_note(1, "beam", [review(4), new()])   # learning + extra NEW
        col.add_note(2, "beam", [review(6), new()])   # learning + extra NEW
        col.add_note(3, "beam", [new(), new()])       # brand-new meaning
        s, u = logic.process_group_sync(col, [1, 2, 3], THRESHOLD, TAG)
        self.assertEqual(s, 2)                         # only note 3's two NEW cards
        for good in (1, 2):
            cards = [col.cards[c] for c in col.note_cards[good]]
            self.assertFalse(any(c.queue == QUEUE_TYPE_SUSPENDED for c in cards))
            self.assertFalse(col.get_note(good).has_tag(TAG))
        self.assertTrue(col.get_note(3).has_tag(TAG))

    def test_sync_heals_a_wrongly_suspended_learning_meaning(self):
        # Regression for the restore bug: a learning meaning that got tagged +
        # NEW-suspended by the old logic must be released on the next scan.
        col = FakeCol()
        col.add_note(1, "beam", [review(4), new()])                       # active learner
        col.add_note(2, "beam", [review(6), new(True)], tags=[TAG])       # wrongly gated
        s, u = logic.process_group_sync(col, [1, 2], THRESHOLD, TAG)
        self.assertEqual((s, u), (0, 1))
        self.assertFalse(col.get_note(2).has_tag(TAG))
        n2 = [col.cards[c] for c in col.note_cards[2]]
        self.assertFalse(any(c.queue == QUEUE_TYPE_SUSPENDED for c in n2))
