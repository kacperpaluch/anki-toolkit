...
SQLite w pamięci. Zmiany UI sprawdzamy także w Anki.

Osobny `workload_service/worker.py` współdzieli `logic.py`, `snapshot.py`
i domyślne wartości z sekcji `workload` w głównym `config.json`,
ale sam zapisuje limity przez API Anki i synchronizuje własną replikę.
Niezmiennik wyłącznie odczytu dotyczy modułu GUI. `logic.py` i `snapshot.py`
muszą pozostać bez importów względnych i bez aqt — usługa ładuje je po ścieżce.
